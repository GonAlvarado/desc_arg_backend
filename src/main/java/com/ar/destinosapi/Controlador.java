...
@WebServlet("/destinos") // Anotación que mapea este servlet a la URL "/peliculas"
public class Controlador extends HttpServlet { // Declaración de la clase Controlador que extiende HttpServlet
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Destino destino = mapper.readValue(request.getInputStream(), Destino.class);  // Convertir el JSON de la solicitud a un objeto Pelicula
        
            // Consulta SQL para insertar una nueva película en la tabla 'peliculas'
            String query = "INSERT INTO destinos (nombre, provincia, descripcion, imagen) VALUES (?, ?, ?, ?)";
            ...
            statement.setString(2, destino.getProvincia());
            statement.setString(3, destino.getDescripcion());
            statement.setString(4, destino.getImagen());
            ...
            if (rs.next()) {
                Long idDest = rs.getLong(1);  // Obtener el valor del primer campo generado automáticamente (en este caso, el ID)
                ...
                String json = mapper.writeValueAsString(idDest);  // Convertir el ID a formato JSON
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            List<Destino> destinos = new ArrayList<>();  // Crear una lista para almacenar objetos Pelicula
            ...
            while (resultSet.next()) {
                ...
                destinos.add(destino);  // Agregar el objeto Pelicula a la lista
            }

            ObjectMapper mapper = new ObjectMapper();  // Crear un objeto ObjectMapper para convertir objetos Java a JSON
            String json = mapper.writeValueAsString(destinos);  // Convertir la lista de películas a formato JSON
